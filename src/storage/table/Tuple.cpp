...
Tuple::Tuple(std::vector<Value> values, const Schema *schema) : allocated_(true) {
  ...
  uint32_t tuple_size = schema->GetLength(); //Fixed length size
  ...
  // 2. Allocate memory
  //This metaDataSize used to reserve  the 16 OverFlowPageId (-1 or valid page id) and 2 bytes NULLBITMAP
  uint32_t metaDataSize = sizeof(uint32_t) + sizeof(uint16_t);
  size_ = tuple_size + metaDataSize;
  data_ = new char[size_];
  std::memset(data_, 0, size_);
  data_ = data_ + sizeof(uint32_t);
  // 3. Serialize each attribute based on the input value.
  uint32_t column_count = schema->GetColumnCount();
  uint32_t offset = schema->GetLength();  
  char *bitMap = data_ + offset;
  for (uint32_t i = 0; i < column_count; i++) { 
    ...
    if (!col.IsInlined()) {
      ...
      *reinterpret_cast<uint32_t *>(data_ + col.GetOffset()) = offset;
      ...
      values[i].SerializeTo(data_ + offset + sizeof(uint16_t));
      auto len = values[i].GetLength();
      if (len == 0) {
        len = 0;
